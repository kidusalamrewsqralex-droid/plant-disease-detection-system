...
with tab1:
    ...
    st.success(f"🌱 You are now using the CNN MobileNetV2 model!!")
    ...
    uploaded_file1 = st.file_uploader("Upload plant image :", type=["jpg", "jpeg", "png"])
...
with tab2:
    ...
    model = load_my_model2()
    st.success(f"🌱 You are now using the CNN model!!")
    st.write("Upload a leaf image and detect the plant disease.")
    st.write("TIP: For best results, use a clean background (black or white).")
    ...
    uploaded_file2= st.file_uploader("Upload an image of your plant:", type=["jpg", "jpeg", "png"])
